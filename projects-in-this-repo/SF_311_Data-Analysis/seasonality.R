...
abandonedVehiclesCalls$Date = as.Date(abandonedVehiclesCalls$Opened)
callsPerDate = aggregate(Opened ~ Date, abandonedVehiclesCalls, length)
callsPerDate.ts = as.ts(callsPerDate)
plot(callsPerDate.ts)
#install.packages(c('forecast', 'xts'))
library(xts)
callsPerDate.xts = xts(callsPerDate$Opened, callsPerDate$Date)
